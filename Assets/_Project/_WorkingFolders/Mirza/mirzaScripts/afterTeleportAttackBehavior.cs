...
public class afterTeleportAttackBehavior : IBossEnemyAttackBehavior
{
    private Vector3 hitboxCenter = new Vector3(0, 1f, 1f);
    private Vector3 hitboxSize = new Vector3(1f, 1f, 1f);
    private LayerMask playerLayer;

    public afterTeleportAttackBehavior()
    {
        
    }
    public void Attack(BossEnemyController controller, Transform target)
    {
        ...
        foreach (Collider hit in hits)
        {
            ...
            if (!damagedEnemies.Contains(enemy))
            {
                ...
                {
                    int minDamage = (int)controller.BoosEnemyData.damage;
                    int maxDamage = (int)controller.BoosEnemyData.damage;
